"""Performance test fixtures for Phaser MCP Server.

This module contains test fixtures and utility functions for performance testing.
"""
...
@pytest.fixture
def mock_server_for_memory_test(large_html_content: str) -> PhaserMCPServer:
    """メモリテスト用のモックサーバーを作成する。

    このフィクスチャは、メモリ使用量テスト用にPhaserMCPServerのモックを作成し、
    必要なメソッドをパッチして実際のHTTPリクエストを行わずにテストできるようにします。

    Args:
        large_html_content: テストに使用する大きなHTMLコンテンツ
